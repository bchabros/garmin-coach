...
def test_features_recomputes_personal_zones(conn):
    ...
    db.upsert_activity(
        ...
        {
            "activity_id": 1,
            "start_local": "2026-06-22 08:00:00",
            "date": "2026-06-22",
            "gtype": "running",
            "discipline": "Bieganie",
            "avg_hr": 150,
    ...
    rows = conn.execute("SELECT lthr_bpm, z2_hi_bpm, source FROM athlete_zones").fetchall()
    assert len(rows) == 1
    assert rows[0][0] == 175
    assert rows[0][1] == 156
...
def test_features_writes_one_athlete_status_row(conn):
    ...
    db.upsert_daily(conn, "training_status_daily", {"date": "2026-06-22", "vo2max": 52.0})
    db.upsert_activity(
        conn,
        {
            "activity_id": 1,
    ...
    rows = conn.execute("SELECT computed_at, vo2max, z2_hi_bpm FROM athlete_status").fetchall()
    assert len(rows) == 1
...
def test_features_rebuilds_plan_block_before_the_weekly_rollup(conn):
    ...
    assert (
        conn.execute("SELECT block FROM weekly_metrics WHERE week_start='2026-06-08'").fetchone()[0]
        == "base"
    )
