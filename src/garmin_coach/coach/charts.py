"""Coach charts: render the two report PNGs from the mart.
...
def render_hrv_band(rows: list[dict], path: pathlib.Path) -> None:
    ...
    if dated:
        ...
        baseline = next(
            (r["hrv_baseline"] for r in dated if r.get("hrv_baseline") is not None), None
        )
        ...
        if low:
            ax.scatter(
                _xnums(low), [r["hrv"] for r in low], color="tab:red", zorder=5, label="low night"
            )
...
def render_acwr(rows: list[dict], path: pathlib.Path, thresholds: dict[str, float]) -> None:
    ...
    if dated:
        ...
        for key, style in (
            ...
            ax.axhline(
                thresholds[key], color="tab:gray", lw=1, ls=style, label=f"{key}={thresholds[key]}"
            )
        unreliable = _xnums(
