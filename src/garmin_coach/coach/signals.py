"""Coach signals: pure functions from mart rows to coach findings.
...
def two_hard_days(rows: list[dict], thresholds: dict[str, float], to_date: str) -> dict | None:
    ...
    for prev, cur in zip(rows, rows[1:]):
        ...
        if (
            consecutive
            and (prev.get("load_day") or 0) >= hard
            and (cur.get("load_day") or 0) >= hard
        ):
            pair = (prev, cur)
    if pair is None:
        return None
...
def hard_rpe_yesterday(rated_rows: list[dict], thresholds: dict[str, float]) -> dict | None:
    """The latest day carried a subjectively hard session.
...
def taper_active(plan: dict | None) -> dict | None:
    ...
    the recommender's decision, deliberately not taken here.
...
def race_proximity(events: list[dict], thresholds: dict[str, float], to_date: str) -> dict | None:
    """A goal race is near - within ``race_proximity_weeks`` of the horizon.
...
def _overlap_signal(
    ...
    day = [
        r for r in overlap_rows if r["dim"] == dim and r["date"] == to_date and r["overlap"] >= high
    ]
    if not day:
        return None
