...
def _series(
    ...
    return [
        ...
        for date, value in conn.execute(
            f"SELECT date, {col} FROM {table} WHERE date <= ? AND {col} IS NOT NULL ORDER BY date",
            (through_date,),
        )
    ]
...
def _markers(conn: sqlite3.Connection, through_date: str, thr: dict[str, float]) -> dict[str, Any]:
    ...
    vo2_delta, vo2_span = _trend(
        vo2,
        through_date,
        thr["snapshot_vo2max_lookback_days"],
    ...
    weight_delta, weight_span = _trend(
        weight,
        through_date,
        thr["snapshot_weight_lookback_days"],
        thr["snapshot_trend_min_span_days"],
    )
    preds = (
        _latest_row(
...
def _hrv(conn: sqlite3.Connection, through_date: str, thr: dict[str, float]) -> dict[str, Any]:
    ...
    read-only series.
    """
    latest = _latest_row(conn, "daily_metrics", "hrv_baseline, hrv_sd", through_date) or {}
    weekly = _series(conn, "hrv_nightly", "weekly_avg", through_date)
    hrv_delta, hrv_span = _trend(
        weekly,
        through_date,
        thr["snapshot_hrv_lookback_days"],
        thr["snapshot_trend_min_span_days"],
    )
    return {
...
def _load_acwr(
    ...
    anaero_share = None if low_share is None or high_share is None else 1 - low_share - high_share
    return {
...
def _recovery(conn: sqlite3.Connection, through_date: str) -> dict[str, Any]:
    ...
    readiness = _latest_row(conn, "training_readiness", "score, level", through_date) or {}
    sleep = _latest_row(conn, "daily_metrics", "sleep_debt_h", through_date) or {}
    accl = (
        _latest_row(
            conn,
            "training_status_daily",
            "heat_accl_pct, heat_trend, altitude_accl",
            through_date,
        )
        or {}
    )
