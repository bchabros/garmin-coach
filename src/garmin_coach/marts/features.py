"""Metrics layer: compute the ``daily_metrics`` mart from core tables.
...
from . import load, overlap, periodize, snapshot, weekly, zones
...
def _recovery_by_day(conn: sqlite3.Connection) -> dict[str, dict[str, int | None]]:
    ...
    for date, score, resting_hr in conn.execute("SELECT date, score, resting_hr FROM sleep"):
