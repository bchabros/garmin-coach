...
Issues and per-feature specs for this repo live as markdown files under
`.scratch/`. There is no external tracker (no GitHub Issues, Jira, etc.).

Note: the formal per-phase PRDs stay where they already are, in `docs/prd/`.
`.scratch/` is the agent working area -- incoming issues, `to-spec` output,
triage state, and `wayfinder` maps. `.scratch/` is committed to git (not
ignored), so this history is shared and durable.
