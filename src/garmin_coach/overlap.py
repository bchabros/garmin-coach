"""Phase 8 movement-overlap mart: the same pattern/muscle loaded on adjacent days.

Pure aggregation over core (``activities``, ``activity_sets``, ``exercise_pattern``)
plus the Phase 7 blended session load. Each session's load is split across its
...
def _session_load(
    ...
    *,
    ...
    """Map each activity to (date, Phase 7 blended load), bounded by through_date."""
    ...
    for aid, date, discipline, dur_s, garmin_load in conn.execute(
        "SELECT activity_id, date(start_local), discipline, dur_s, training_load "
        "FROM activities"
    ):
        ...
        loads[aid] = (
            date,
            load.activity_load(
                discipline, garmin_load, rpe_by_activity.get(aid), dur_s,
