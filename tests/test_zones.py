...
from garmin_coach import db, zones
...
def _seed_lthr(conn, date="2026-07-02", hr=175, pace=257.1):
    db.upsert_daily(conn, "fitness_markers", {"date": date, "lactate_thr_hr": hr,
                                              "lactate_thr_pace": pace})


def _seed_run(conn, aid, date, speed_mps, hr, temp_c=15.0):
    db.upsert_activity(conn, {
        "activity_id": aid, "start_local": f"{date} 08:00:00", "date": date,
        "gtype": "running", "discipline": "Bieganie", "avg_hr": hr,
        "avg_speed_mps": speed_mps, "temp_c": temp_c,
    })
...
def test_rollup_writes_single_zones_row_from_core(conn):
    ...
    rows = conn.execute(
        "SELECT lthr_bpm, z2_hi_bpm, source, stale FROM athlete_zones"
    ).fetchall()
    assert len(rows) == 1
