...
Schema is shipped as a package resource (src/garmin_coach/core/schema.sql) and loaded
...
_ADDED_COLUMNS: dict[str, dict[str, str]] = {
    "activities": {"temp_c": "REAL"},  # per-activity temperature (personal zones)
    "daily_metrics": {"load_strength": "REAL"},  # blended strength load
    "weekly_metrics": {  # weekly strength load + its share
        "load_strength": "REAL",
        "strength_share": "REAL",
        # periodization: the week's block, copied from plan_block
        "block": "TEXT",
...
def bootstrap_sync_watermark(
    ...
    watermark = (
        row[0]
        if row and row[0]
        else (_dt.date.fromisoformat(data_start_date) - _dt.timedelta(days=1)).isoformat()
    )
