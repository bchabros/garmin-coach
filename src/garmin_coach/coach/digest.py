"""Coach engine: build a compact ``digest`` from the mart.
...
from ..marts import zones as _zones

DISCLAIMER = "This is a reading of your recorded data, not medical or coaching advice."
...
def enrich_hrv_band(rows: list[dict], thresholds: dict[str, float]) -> list[dict]:
    ...
    reading but no computed band (e.g. too few nights for the mart to compute one).
    """
...
def build_digest(
    ...
    *,
    ...
    result["recommendation"] = _recommend.recommend(result, _planned_intent(conn, to_date), thr)
    return result
...
def _lthr_age_days(detected_on: str | None, computed_at: str | None) -> int | None:
    ...
    if not detected_on or not computed_at:
        return None
    return (_dt.date.fromisoformat(computed_at) - _dt.date.fromisoformat(detected_on)).days
