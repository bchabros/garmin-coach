"""The coach MCP server: a thin protocol layer over ``mcp_tools`` (epic #18).
...
``mcp_tools``, and returns its freshness-enveloped dict. The only tools that
touch Garmin are ``refresh_today`` (a read through the transport seam) and
``push_preview``/``push_confirm`` (the Phase 11 outbound path behind the
...
from . import client, db, mcp_tools, publish
...
def get_snapshot() -> dict[str, Any]:
    ...
    conn = _open()
    try:
        return mcp_tools.get_snapshot(conn)
    finally:
        conn.close()
...
def get_digest(to_date: str | None = None) -> dict[str, Any]:
    ...
    conn = _open()
    try:
        return mcp_tools.get_digest(conn, to_date=to_date)
    finally:
        conn.close()
...
def get_recent_activities(n: int = 10) -> dict[str, Any]:
    ...
    conn = _open()
    try:
        return mcp_tools.get_recent_activities(conn, n=n)
    finally:
        conn.close()
...
def get_weekly(week_start: str | None = None) -> dict[str, Any]:
    ...
    conn = _open()
    try:
        return mcp_tools.get_weekly(conn, week_start=week_start)
    finally:
        conn.close()
...
def get_zones() -> dict[str, Any]:
    ...
    conn = _open()
    try:
        return mcp_tools.get_zones(conn)
    finally:
        conn.close()
...
def get_recommendation(date: str | None = None) -> dict[str, Any]:
    ...
    conn = _open()
    try:
        return mcp_tools.get_recommendation(conn, date=date)
    finally:
        conn.close()
...
def get_events(today: str | None = None) -> dict[str, Any]:
    ...
    conn = _open()
    try:
        return mcp_tools.get_events(conn, today=today)
    finally:
        conn.close()
...
def get_workout_status(date: str) -> dict[str, Any]:
    ...
    conn = _open()
    try:
        return mcp_tools.get_workout_status(conn, date=date, reports_dir=_REPORTS_DIR)
    finally:
        conn.close()
...
def log_niggle(
    ...
    conn = _open()
    try:
        return mcp_tools.log_niggle(
            conn, body_part=body_part, severity=severity, date=date, note=note
        )
    finally:
        conn.close()
...
def refresh_today() -> dict[str, Any]:
    ...
    try:
        ...
        return mcp_tools.refresh_today(conn, transport, data_start_date=settings.data_start_date)
    finally:
        conn.close()
...
def author_workout(
    ...
    conn = _open()
    try:
        return mcp_tools.author_workout(
...
def push_preview(date: str) -> dict[str, Any]:
    ...
    try:
        ...
        return mcp_tools.push_preview(
            conn, date=date, publisher=publisher, reports_dir=_REPORTS_DIR
        )
    finally:
        conn.close()
