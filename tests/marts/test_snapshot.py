...
def _seed_full_standing(conn):
    ...
    db.upsert_zones(
        conn,
        {
            ...
            "z2_pace_ceiling_s_per_km": 334.23,
            "source": "regression+lthr",
            "lthr_detected_on": "2026-07-02",
    ...
    db.upsert_daily(conn, "training_status_daily", {"date": "2026-06-20", "vo2max": 51.0})
    db.upsert_daily(conn, "training_status_daily", {"date": "2026-07-07", "vo2max": 52.0})
    ...
    db.upsert_daily(
        conn, "training_readiness", {"date": "2026-07-06", "score": 60, "level": "MODERATE"}
    )
    db.upsert_daily(
        conn, "training_readiness", {"date": "2026-07-08", "score": 72, "level": "HIGH"}
    )
    db.upsert_daily(
        conn,
...
def test_full_standing_composes_every_direct_read_field(conn):
    ...
    assert (s["z1_hi_bpm"], s["z2_hi_bpm"], s["z3_hi_bpm"], s["z4_hi_bpm"]) == (140, 156, 164, 173)
...
def test_trend_delta_is_null_below_the_min_span(conn):
    ...
    db.upsert_daily(conn, "training_status_daily", {"date": "2026-07-04", "vo2max": 51.0})
    db.upsert_daily(conn, "training_status_daily", {"date": "2026-07-08", "vo2max": 52.0})
    s = snapshot.build(conn, through_date="2026-07-08")
...
def test_trend_span_is_the_available_history_not_the_lookback(conn):
    ...
    db.upsert_daily(conn, "training_status_daily", {"date": "2026-06-14", "vo2max": 50.0})
    db.upsert_daily(conn, "training_status_daily", {"date": "2026-07-08", "vo2max": 52.0})
    s = snapshot.build(conn, through_date="2026-07-08")
...
def test_no_zones_anchor_degrades_without_raising(conn):
    ...
    _seed_daily(
        conn, "2026-07-08", load_day=100, low=60, high=30, anaero=10, acwr=1.0, n_chronic=10
    )
    s = snapshot.build(conn, through_date="2026-07-08")
...
def test_rollup_writes_single_status_row(conn):
    ...
    rows = conn.execute(
        "SELECT computed_at, vo2max, acwr, z2_hi_bpm, planned_intent_today FROM athlete_status"
    ).fetchall()
    assert len(rows) == 1
